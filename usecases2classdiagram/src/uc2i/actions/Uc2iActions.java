...
public class Uc2iActions implements VPActionController {
	...
	public void performAction(VPAction arg0) {
		...
		for (int i = 0; i < actors.length; i++) {
			...
			if (actor.hasStereotype("external")) {Class.addStereotype("external");}
			else
				if (actor.hasStereotype("scenario")) {Class.addStereotype("scenario");}
				else { Class.addStereotype("Actor");}
		...
		for (int i = 0; i < actors.length; i++) {
			...
			while (genIter.hasNext()) {
				...
				//TRANSIT TO
				ITransitProperty transitProp = (ITransitProperty) relationship.getModelPropertyByName(IModel.PROP_TRANSIT_TO); 
		...
		for (int i = 0; i < usecases.length; i++) {
			...
			if (useCase.hasStereotype("business")) {
			}
			else {
				IClass Class = IModelElementFactory.instance().createClass(); 
				interfacePackage.addChild(Class);
		...
		for (int i = 0; i < usecases.length; i++) {
			...
			while (genIter.hasNext()) {
				...
				if (relationship.getModelType() == "Generalization") {
					...
					if (usecase.hasStereotype("list")) {
					IGeneralization generalizationModelitem = IModelElementFactory.instance().createGeneralization();
					...
					//TRANSIT TO
					ITransitProperty transitProp2 = (ITransitProperty) relationship.getModelPropertyByName(IModel.PROP_TRANSIT_TO); 
		...
		for (int i = 0; i < actors.length; i++) {
			...
			if (tousecase.hasStereotype("option")) { list="";} else {list="_item";}
			}
			...
			if (tousecase.hasStereotype("event")) {
				...
				if (tousecase.hasStereotype("business")) {
					String id_from = actors_id.get(fromactor.getName().replaceAll(" ", "_")+list);
					IClass actor_from = (IClass) projectManager.getProject().getModelElementById(id_from);
					IOperation event_handler = IModelElementFactory.instance().createOperation();
					event_handler.setName(tousecase.getName().replaceAll(" ", "_"));
